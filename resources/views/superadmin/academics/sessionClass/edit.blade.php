                ...
                <div class="col-xl-12 col-xxl-12 col-sm-12">
                    <div class="card">
                        <div class="card-header">
                            <h5 class="card-title">Edit Class for Session</h5>
                        </div>
                        <div class="card-body">
                            ...
                            <form action="
                                ...
                                @if(auth()->guard('staff')->user()->role->name == 'Admin')
                                    {{route('admin.session-class.update')}}
                                @elseif(auth()->guard('staff')->user()->role->name == 'Teacher')
                                    {{route('teacher.session-class.update')}}
                                @endif
                                @elseif(\Illuminate\Support\Facades\Auth::user()->user_type == 'superadmin')
                                    {{route('session-class.update')}}
                                @endif
                                " method="POST" id="sessionform">
                                <input type="hidden" name='id' value={{$sessionClass['id']}}>
                                @csrf
                                <div class="row">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="session_id">
                                                ...
                                                @foreach ($session as $row)
                                                    <option value='{{ $row->id }}' {{ (collect($sessionClass->session_id)->contains($row->id)) ? 'selected':'' }}>{{$row->session_year}}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="class_id" id="class_id">
                                                ...
                                                @foreach ($class as $row)
                                                    <option value='{{ $row->id }}' {{ (collect($sessionClass->class_id)->contains($row->id)) ? 'selected':'' }}>{{$row->name}}</option>
                                                @endforeach
                                            </select>
