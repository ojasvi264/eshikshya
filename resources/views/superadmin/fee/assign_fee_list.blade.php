            ...
            <div class="row page-titles mx-0">
                <div class="col-sm-6 p-md-0">
                    <div class="welcome-text">
                        <h4>Assigned Fee For Students</h4>
                    </div>
                </div>
...
{{--                                @if(\Illuminate\Support\Facades\Auth::guard('staff')->check())--}}
                            ...
                            {{route('assigned_fee_student.search')}}
                            {{--                            @endif--}}
                                " method="get">
                                @csrf
                                    ...
                                    <div class="table-responsive">
                                        <table id="example3" class="display" style="min-width: 845px">
                                            ...
                                            @foreach($assignedFeeStudents as $assignedStudent)
                                                <tr>
                                                    <td>{{$i++}}</td>
                                                    <td>{{$assignedStudent->fee_master->fees_type->name}}
                                                        @if($assignedStudent->month_name != "")
                                                            ({{$assignedStudent->month_name}})
                                                        @endif
                                                    </td>
                                                    <td>{{$assignedStudent->fee_master->fee_group->name}}</td>
                                                    <td>{{$assignedStudent->student->fname}}</td>
                                                    <td>{{$assignedStudent->student->class->name}}</td>
                                                    <td>{{$assignedStudent->student->section->name}}</td>
                                                    <td>{{$assignedStudent->student->admission}}</td>
                                                    <td>{{$assignedStudent->student->phone}}</td>
                                                    <td>
                                                        <form action="{{route('assigned_fee_student.destroy', $assignedStudent->id)}}" method="post" onsubmit="return confirm('Are you sure?')">
                                                            @method('delete')
                                                            @csrf
