...
if(!function_exists('getGrade')){
    function getGrade($marks_obtained, $grades)
    {
        foreach ($grades as $row) {
            if ($marks_obtained >= $row['per_from'] && $marks_obtained <= $row['per_to'])
                return $row;
        }
    }
}
...
if(!function_exists('getPromotionFromGradePoint2')){
    ...
    {
        $class = \App\Models\Eclass::find($class_id);
        if($class_id == 12){
            $upper_class = \App\Models\Eclass::find(14);
        }else{
            $upper_class = \App\Models\Eclass::find($class_id + 1);
        }
        if($grade_point > 3.6 && $grade_point <= 4.0){
...
if(!function_exists('getRemarksFromPercentage')){
    function getRemarksFromPercentage($percentage)
    {
